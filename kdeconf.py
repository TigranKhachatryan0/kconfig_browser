...
class KDEConfig:
    ...
    def get_list_of_config_files(self, dir: str = str(os.path.expanduser('~/.config'))):
        return sorted([f for f in os.listdir(dir) if os.path.isfile(os.path.join(dir, f)) and f.endswith("rc") and f not in ("gtkrc",)], key = str.casefold)
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _generate_config_dictionary(self):
        ...
        config_files_paths = [os.path.expanduser(f'~/.config/{c}') for c in config_files]
        config_files_dict = {}
